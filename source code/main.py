...
if mode is None:
    ...
    url = build_url({'mode': 'folder', 'foldername': 'category_1'})
    li = xbmcgui.ListItem('category_1', iconImage=categories_icon)
    li.setArt({'fanart':fanart_main})
    url2 = build_url({'mode':'settings', 'foldername': 'settings'})
    settings=(status_login, 'XBMC.RunPlugin('+url2+')')  
    li.addContextMenuItems([settings] ,replaceItems=True)
    ...
    director='Ian Hubert'
    castfull="Derek de Lint, Sergio Hasselbaink, Rogier Schippers, Vanja Rukavina, Denise Rebergen, Jody Bhe, Chris Haley"
    ...
    if how_many==3:
    	...
    	seconds=int(duration2[2])
    if how_many==2: 
    	minutes=int(duration2[0])
    	seconds=int(duration2[1])
    if how_many==1: 
    	seconds=int(duration2[0])
